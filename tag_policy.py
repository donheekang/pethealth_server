...
#
# 목표:
# - import-time에서 외부 SDK로 죽지 않게
# - 짧은 약어 오탐 방지(us/ua/pi 등)
# - Rabies(광견병) "ra"만 떠도 태깅되게 강제(요구사항 반영)
# - 절대 예외로 서버를 죽이지 않게 (resolve_record_tags는 항상 dict 반환)
...
def _match_score(tag: Dict[str, Any], query: str) -> Tuple[int, Dict[str, Any]]:
    ...
    for alias in tag.get("aliases", []):
        ...
        # 짧은 약어(us/ua/pi/l2 등)은 contains 오탐이 커서 eq/token만 허용
        ...
        if a_norm == q_norm:
            ...
            best = max(best, 120 + min(60, len(a_norm) * 2))
            hit += 1
            why.append(f"inQuery:{a}")
        elif a_norm.find(q_norm) >= 0:
            best = max(best, 90 + min(40, len(q_norm) * 2))
            hit += 1
...
def _build_record_query(items: List[Dict[str, Any]], hospital_name: Optional[str], ocr_text: Optional[str] = None) -> str:
    ...
    # OCR 원문 일부를 섞어서 인식률 보강 (너무 길면 오탐↑)
...
def _find_partial_rabies_item_idxs(items: List[Dict[str, Any]]) -> Tuple[List[int], List[str]]:
    ...
    for i, it in enumerate((items or [])[:200]):
        ...
        hit += list(tn.intersection(_RABIES_PARTIAL))
...
def _detect_rabies_force(items: List[Dict[str, Any]], hospital_name: Optional[str], ocr_text: Optional[str]) -> Dict[str, Any]:
    combined = " | ".join([
        hospital_name or "",
        ocr_text or "",
    ...
    if _contains_strong_rabies(combined):
        return {"force": True, "confidence": 1.0, "reason": "strong_text", "itemIdxs": [], "partialTokens": [], "hasVaccineContext": has_ctx}
    ...
    if not union_tokens:
        return {"force": False, "confidence": 0.0, "reason": "no_signal", "itemIdxs": [], "partialTokens": [], "hasVaccineContext": has_ctx}

    # ✅ 요구사항: "ra만 있어도 rabies" => 컨텍스트 없어도 force True
    only_ra = (union_tokens == ["ra"])
    if only_ra and not has_ctx:
        return {"force": True, "confidence": 0.6, "reason": "partial_ra_no_ctx_force", "itemIdxs": idxs, "partialTokens": union_tokens, "hasVaccineContext": has_ctx}

    conf = 0.9 if has_ctx else 0.8
    if only_ra and has_ctx:
        conf = 0.85
    return {"force": True, "confidence": conf, "reason": "partial_tokens", "itemIdxs": idxs, "partialTokens": union_tokens, "hasVaccineContext": has_ctx}
...
def resolve_record_tags(
    ...
    **kwargs,
    ...
    try:
        ...
        if ocr_text is None:
            ocr_text = kwargs.get("ocrText")  # 호환
        ...
        rab = _detect_rabies_force(items or [], hospital_name, ocr_text)
        ...
        if return_item_tags:
            ...
            # Rabies item override (partial token item들)
            if rab.get("force") is True:
                for idx in (rab.get("itemIdxs") or []):
        ...
        # 절대 서버를 죽이지 않게 안전 반환
        return {"tags": [], "itemCategoryTags": [], "evidence": {"policy": "catalog", "reason": "exception", "error": (str(e) or repr(e))[:200]}}
