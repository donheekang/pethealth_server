...
def _match_score(tag: Dict[str, Any], query: str) -> Tuple[int, Dict[str, Any]]:
    ...
    for alias in tag.get("aliases", []):
        ...
        # ✅ 짧은 약어(us/ua/pi/l2)는 contains 오탐이 크니 token/eq만 허용
...
def _build_record_query(items: List[Dict[str, Any]], hospital_name: Optional[str], ocr_text: Optional[str] = None) -> str:
    ...
    for it in (items or [])[:120]:
        ...
        if nm:
            parts.append(nm)
    # ✅ OCR 원문도 조금만 섞어서 “어떤 상황이든” 인식률 올림 (너무 길면 오탐↑)
    if isinstance(ocr_text, str) and ocr_text.strip():
        parts.append((ocr_text or "")[:1200])
    return " | ".join(parts)

# -----------------------------
# ✅ Rabies(광견병) 보강: "ra"만 있어도 vaccine_rabies로 태깅
# -----------------------------
...
def _find_partial_rabies_item_idxs(items: List[Dict[str, Any]]) -> Tuple[List[int], List[str]]:
    ...
    for i, it in enumerate((items or [])[:200]):
        ...
        nm_norm = _normalize(nm)  # "r a" -> "ra" 같은 케이스도 포함
        hit2: List[str] = []
...
def _find_partial_rabies_in_text(ocr_text: Optional[str]) -> Tuple[bool, List[str], List[str]]:
    ...
    all_tokens = sorted(list(set(hit_tokens + [_normalize(x) for x in matched_lines if _normalize(x) in _RABIES_PARTIAL])))
    return found, all_tokens, matched_lines[:8]

def _gemini_bool(*, prompt: str, api_key: str, model: str, timeout_seconds: int) -> Optional[bool]:
    try:
    ...
    *,
    ...
    if only_ra and (not has_ctx):
        ...
        if ge and gk:
            ...
            b = _gemini_bool(prompt=prompt, api_key=gk, model=gm, timeout_seconds=gt)
            if b is True:
            ...
            if b is False:
                # 그래도 태깅은 True로 (요구사항 우선) + confidence 낮게
...
def resolve_record_tags(
    ...
    **kwargs,
    ...
    gemini_enabled = kwargs.get("gemini_enabled")
    gemini_api_key = kwargs.get("gemini_api_key")
    gemini_model_name = kwargs.get("gemini_model_name")
    gemini_timeout_seconds = kwargs.get("gemini_timeout_seconds")
    ...
    rab = _detect_rabies_force(
        ...
        ocr_text=ocr_text,
        gemini_enabled=gemini_enabled if isinstance(gemini_enabled, bool) else None,
        gemini_api_key=gemini_api_key if isinstance(gemini_api_key, str) else None,
        gemini_model_name=gemini_model_name if isinstance(gemini_model_name, str) else None,
        gemini_timeout_seconds=int(gemini_timeout_seconds) if isinstance(gemini_timeout_seconds, (int, float, str)) and str(gemini_timeout_seconds).strip().isdigit() else None,
    )
