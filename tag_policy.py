...
TAG_CATALOG: List[Dict[str, Any]] = [
    ...
    {"code": "exam_fecal", "group": "exam", "aliases": ["fecal","stool test","대변검사","분변검사"]},
    ...
    {"code": "vaccine_lepto", "group": "vaccine", "aliases": ["lepto","leptospirosis","lepto2","lepto4","l2","l4","렙토","렙토2","렙토4"]},

    # --- Preventives ---
    {"code": "prevent_heartworm", "group": "preventive_med", "aliases": ["heartworm","hw","heartgard","심장사상충","하트가드","레볼루션","넥스가드스펙트라","simparica trio"]},
    {"code": "prevent_external", "group": "preventive_med", "aliases": ["flea","tick","bravecto","nexgard","frontline","revolution","벼룩","진드기","외부기생충"]},
    {"code": "prevent_deworming", "group": "preventive_med", "aliases": ["deworm","drontal","milbemax","fenbendazole","구충","구충제","내부기생충"]},

    # --- Medicines ---
    {"code": "medicine_antibiotic", "group": "medicine", "aliases": ["antibiotic","abx","amoxicillin","clavamox","augmentin","cephalexin","convenia","doxycycline","metronidazole","항생제"]},
    {"code": "medicine_anti_inflammatory", "group": "medicine", "aliases": ["nsaid","anti-inflammatory","meloxicam","metacam","carprofen","rimadyl","onsior","소염","소염제"]},
    ...
    # --- Care / General ---
    {"code": "care_injection", "group": "checkup", "aliases": ["inj","injection","shot","iv","im","sc","주사","주사료","피하주사","근육주사","정맥주사"]},
...
def _match_score(tag: Dict[str, Any], query: str) -> Tuple[int, Dict[str, Any]]:
    ...
    for alias in tag.get("aliases", []):
        ...
        # short tokens: only eq or token match (contains 금지)
...
def resolve_record_tags(
    ...
    **kwargs,
    ...
    evidence: Dict[str, Any] = {
        ...
        "candidates": [{"code": c, "score": sc, **(ev or {})} for (c, sc, ev) in scored[:15]],
    }
    ...
    if return_item_tags:
        for idx, it in enumerate((items or [])[:200]):
            ...
            # hard override for rabies spelling variants
            low = nm.lower()
            if ("rabies" in low) or ("rabbies" in low) or ("광견병" in nm) or ("광견" in nm):
                item_tags.append({"idx": idx, "itemName": nm, "categoryTag": "vaccine_rabies", "score": 999, "why": ["hard_rabies"]})
                continue
            ...
            if best_code and best_score >= int(item_thresh) and best_code != "etc_other":
