...
# ------------------------------------------------
# 7.1 ✅ Postgres DB (Render Postgres) - users / pets / visits
# ------------------------------------------------

_db_pool: Optional[SimpleConnectionPool] = None


def _parse_admin_uids(raw: str) -> Set[str]:
...
def init_db_pool() -> None:
    """
    DATABASE_URL이 설정된 경우에만 커넥션 풀을 초기화합니다.
    """
    global _db_pool
    if _db_pool is not None:
        return
    ...
    if not dsn:
        print("[DB] DATABASE_URL is empty. DB endpoints will be unavailable.")
        return

    try:
        _db_pool = SimpleConnectionPool(
            int(settings.DB_POOL_MIN),
            int(settings.DB_POOL_MAX),
            dsn=dsn,
            connect_timeout=8,
        )
        print("[DB] Postgres pool initialized.")
    except Exception as e:
        print("[DB] Failed to initialize Postgres pool:", e)
        _db_pool = None
...
def db_cursor():
    """
    psycopg2 connection pool에서 connection을 빌려서,
    RealDictCursor로 dict row를 반환하도록 cursor를 제공.
    """
    init_db_pool()
    if _db_pool is None:
        raise HTTPException(status_code=500, detail="DB not configured: DATABASE_URL is empty or invalid")
...
def _require_admin(user: Dict[str, Any] = Depends(get_current_user)) -> Dict[str, Any]:
    """
    ADMIN_UIDS에 포함된 UID만 관리자 API 접근 허용
    """
    if settings.STUB_MODE.lower() == "true":
        return user

    uid = (user.get("uid") or "").strip()
    allowed = _parse_admin_uids(settings.ADMIN_UIDS)
    if not allowed:
        raise HTTPException(status_code=403, detail="ADMIN_UIDS is empty. Admin endpoints are disabled.")
    if uid not in allowed:
        raise HTTPException(status_code=403, detail="Admin only.")
    return user


def _to_uuid(v: str, field: str = "id") -> uuid.UUID:
    try:
        return uuid.UUID(str(v))
    except Exception:
        raise HTTPException(status_code=400, detail=f"Invalid UUID for {field}: {v}")


def _to_date(v: Optional[str], field: str) -> Optional[date]:
    if v is None:
        return None
    s = str(v).strip()
    if not s:
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(status_code=400, detail=f"Invalid date for {field}: {v}")
...
class PetUpsertRequest(BaseModel):
    # 클라이언트가 UUID를 갖고 있으면 넣어도 되고(권장), 없으면 생략해도 됨
    id: Optional[str] = None
    name: str
    species: str
...
class VisitUpsertRequest(BaseModel):
    # ✅ Swift에서 camelCase로 보내도 되고, snake_case로 보내도 됨
    id: Optional[str] = Field(default=None, validation_alias=AliasChoices("id"))
    pet_id: str = Field(validation_alias=AliasChoices("petId", "pet_id"))
    visited_at: str = Field(validation_alias=AliasChoices("visitedAt", "visited_at"))
    hospital_name: Optional[str] = Field(default=None, validation_alias=AliasChoices("hospitalName", "hospital_name"))
    total_cost: int = Field(validation_alias=AliasChoices("totalCost", "total_cost"))
    memo: Optional[str] = None
...
def db_health():
    init_db_pool()
    if _db_pool is None:
        return {"ok": False, "detail": "DB pool not initialized. Check DATABASE_URL."}
    try:
        with db_cursor() as cur:
            cur.execute("SELECT 1 as ok;")
            row = cur.fetchone()
        return {"ok": True, "db": "postgres", "result": row}
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"DB health check failed: {e}")


# ---------- 1) 유저 upsert ----------
# POST /api/db/user/upsert
@app.post("/api/db/user/upsert", response_model=DBUserDTO)
def db_user_upsert(user: Dict[str, Any] = Depends(get_current_user)):
    firebase_uid = (user.get("uid") or "").strip()
    if not firebase_uid:
        raise HTTPException(status_code=401, detail="Missing uid")

    with db_cursor() as cur:
        # users.firebase_uid가 PK 또는 UNIQUE 라는 전제 (권장)
        cur.execute(
            """
            INSERT INTO users (firebase_uid)
            VALUES (%s)
...
# ---------- 2) pets upsert ----------
# POST /api/db/pets/upsert
@app.post("/api/db/pets/upsert", response_model=PetDTO)
def db_pets_upsert(
    ...
    species = (body.species or "").strip()
    if not name or not species:
        raise HTTPException(status_code=400, detail="name/species are required")

    birthday_dt = _to_date(body.birthday, "birthday")

    with db_cursor() as cur:
        # ✅ id가 있으면 해당 id로 upsert (권장: 앱의 로컬 UUID를 그대로 보내기)
        if body.id:
            pet_uuid = _to_uuid(body.id, "id")
            cur.execute(
                """
                ...
                """,
                (pet_uuid, uid, name, species, birthday_dt),
            )
            row = cur.fetchone()
            if not row:
        ...
        # ✅ id가 없으면 새로 insert (중복 이름 가능)
        cur.execute(
            """
            INSERT INTO pets (user_uid, name, species, birthday)
            VALUES (%s, %s, %s, %s)
            RETURNING id, user_uid, name, species, birthday, created_at;
            """,
            (uid, name, species, birthday_dt),
        )
        row = cur.fetchone()

    if not row:
        raise HTTPException(status_code=500, detail="Pet insert failed")
    return row


# ---------- 3) pets list ----------
# GET /api/db/pets/list
@app.get("/api/db/pets/list", response_model=List[PetDTO])
def db_pets_list(user: Dict[str, Any] = Depends(get_current_user)):
    uid = (user.get("uid") or "").strip()
    if not uid:
        raise HTTPException(status_code=401, detail="Missing uid")
    ...
    with db_cursor() as cur:
        cur.execute(
            """
            SELECT id, user_uid, name, species, birthday, created_at
            FROM pets
            WHERE user_uid = %s
...
def _assert_pet_owned(cur, uid: str, pet_id: uuid.UUID) -> None:
    cur.execute(
        "SELECT 1 FROM pets WHERE id = %s AND user_uid = %s;",
        (pet_id, uid),
    )
...
# ---------- 4) visits upsert ----------
# POST /api/db/visits/upsert
@app.post("/api/db/visits/upsert", response_model=VisitDTO)
def db_visits_upsert(
    ...
    pet_uuid = _to_uuid(body.pet_id, "petId")
    visited_dt = _to_date(body.visited_at, "visitedAt")
    if visited_dt is None:
        raise HTTPException(status_code=400, detail="visitedAt is required")

    hospital = (body.hospital_name or None)
    memo = (body.memo or None)
    total_cost = int(body.total_cost)

    with db_cursor() as cur:
        _assert_pet_owned(cur, uid, pet_uuid)

        if body.id:
            visit_uuid = _to_uuid(body.id, "id")
            cur.execute(
                """
                ...
                """,
                (visit_uuid, uid, pet_uuid, visited_dt, hospital, total_cost, memo),
            )
            row = cur.fetchone()
            if not row:
                raise HTTPException(status_code=404, detail="Visit not found (or not owned by user)")
            return row

        cur.execute(
            """
            INSERT INTO visits (user_uid, pet_id, visited_at, hospital_name, total_cost, memo)
            VALUES (%s, %s, %s, %s, %s, %s)
            RETURNING id, user_uid, pet_id, visited_at, hospital_name, total_cost, memo, created_at;
            """,
            (uid, pet_uuid, visited_dt, hospital, total_cost, memo),
        )
        row = cur.fetchone()
...
# ---------- 5) visits list ----------
# GET /api/db/visits/list?petId=...
@app.get("/api/db/visits/list", response_model=List[VisitDTO])
def db_visits_list(
    petId: str = Query(...),
    user: Dict[str, Any] = Depends(get_current_user),
):
...
def admin_stats(admin: Dict[str, Any] = Depends(_require_admin)):
    with db_cursor() as cur:
        cur.execute("SELECT COUNT(*)::int AS users FROM users;")
        users_count = (cur.fetchone() or {}).get("users", 0)

        cur.execute("SELECT COUNT(*)::int AS pets FROM pets;")
        pets_count = (cur.fetchone() or {}).get("pets", 0)

        cur.execute("SELECT COUNT(*)::int AS visits, COALESCE(SUM(total_cost),0)::bigint AS total_cost_sum FROM visits;")
        row = cur.fetchone() or {}
        visits_count = row.get("visits", 0)
        total_cost_sum = int(row.get("total_cost_sum", 0))
    ...
    return {
        ...
        "pets": pets_count,
        "visits": visits_count,
        "totalCostSum": total_cost_sum,
        "updatedAt": datetime.utcnow().isoformat(),
    }


@app.get("/api/admin/users")
def admin_users(admin: Dict[str, Any] = Depends(_require_admin)):
    with db_cursor() as cur:
        cur.execute(
            """
            SELECT
                u.firebase_uid,
                u.created_at,
                COALESCE(p.pet_count, 0)::int AS pet_count,
                COALESCE(v.visit_count, 0)::int AS visit_count,
                COALESCE(v.total_cost_sum, 0)::bigint AS total_cost_sum,
                v.last_visited_at
            FROM users u
            LEFT JOIN (
                SELECT user_uid, COUNT(*) AS pet_count
                FROM pets
                GROUP BY user_uid
            ) p ON p.user_uid = u.firebase_uid
            LEFT JOIN (
                SELECT user_uid, COUNT(*) AS visit_count, COALESCE(SUM(total_cost), 0) AS total_cost_sum, MAX(visited_at) AS last_visited_at
                FROM visits
                GROUP BY user_uid
    ...
    # datetimes/date는 FastAPI가 ISO string으로 변환
    return {"items": rows}
