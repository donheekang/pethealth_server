...
def api_ai_analyze(req: AICareAnalyzeRequest, user: Dict[str, Any] = Depends(get_current_user)):
    ...
    prompt_lines.extend([
        ...
        "2. summary는 한국어 4~6문장으로 작성. 보호자에게 따뜻하게 조언하는 말투(~해요 체).",
        "   - 반드시 '{이름} 보호자님,'으로 시작할 것.",
        "   - 현재 건강 상태 평가 → 최근 진료/예방 이력 언급 → 앞으로의 관리 조언 순서로 작성.",
        "   - 딱딱한 보고서가 아니라 수의사가 보호자에게 직접 이야기하듯 친근하게 작성.",
        "   - 예시: '{이름} 보호자님, {이름}는 현재까지 건강 기록이 잘 관리되고 있는 것으로 보여요. 가장 최근에는 2025년 11월 28일에 광견병 예방접종을 마쳤어요. 최근 진료 이력이 없는 것으로 보아 특별한 건강 문제가 발생하지 않고 건강하게 잘 지내고 있네요. 지금처럼 정기적인 건강 관리에 신경 써주시면 {이름}가 앞으로도 건강하게 지낼 거예요.'",
